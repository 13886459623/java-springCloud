...
public class ConfigController {
	...
	public AjaxObject restartA(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
            result = configService.restartA();
            returnVal.setMessage(result);
		} catch (Exception e) {
    ...
    @RequestMapping(value="/configA/getConfigAproduct")
    @ResponseBody
    public Map<String, Object> getConfigAproduct(HttpServletRequest request, HttpServletResponse response) {
        Map<String, Object> returnMap = new HashMap<>();
        try {
            List<config_a> list = configService.getConfigAproduct();
            returnMap.put("data", list);
        }catch (Exception e){
            logger.error("查询数据异常:" + e);
        }
    ...
    @RequestMapping(value="/configA/updateAProductSwitch")
    @ResponseBody
    public AjaxObject updateAProductSwitch(HttpServletRequest request, HttpServletResponse response) throws Exception {
	    AjaxObject returnVal = new AjaxObject();
        Map<String, Object> params = CommonUtil.getParameterMap(request);
        try {
            String result = configService.updateAProductSwitch(params);
			returnVal.setMessage(result);
        }catch (Exception e){
    ...
    @RequestMapping(value="/configB/updateBProductSwitch")
    @ResponseBody
    public AjaxObject updateBProductSwitch(HttpServletRequest request, HttpServletResponse response) throws Exception {
	    AjaxObject returnVal = new AjaxObject();
        Map<String, Object> params = CommonUtil.getParameterMap(request);
        try {
            String result = configService.updateBProductSwitch(params);
			returnVal.setMessage(result);
        }catch (Exception e){
            logger.error("更新数据异常:" + e);
        }
	...
	@RequestMapping(value="/configB/getConfigBproduct")
	@ResponseBody
	public Map<String, Object> getConfigBproduct(HttpServletRequest request, HttpServletResponse response) {
		Map<String, Object> returnMap = new HashMap<>();
		try {
			List<config_b> list = configService.getConfigBproduct();
			returnMap.put("data", list);
		}catch (Exception e){
			logger.error("查询数据异常:" + e);
		}
