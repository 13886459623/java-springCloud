...
function TableInti(){
	...
	var TGM = table.GM({
		...
		,supportSorting: true
		,ajax_url: '${pageContext.request.contextPath }/configuration/user/list'
		,ajax_type: 'POST'
		,query: {}   //参数
		,pageSize:20
/* 		// ajax请求返回的列表数据key键值,默认为data
		...
		,columnData: [{
				key: 'elId',
				remind: '',
				align: 'left',// 三种值: 'left', 'center', 'right'
				isShow: true, //是否显示
				width: '50px',
				text: '编号'
			},
			{
				...
				text: '创建时间',
				sorting: 'DESC',
				template: function(elBadtypetime, rowObject){
					return new Date(elBadtypetime).format('YYYY-MM-DD HH:mm:ss');
				}
			},{
				key: '',
				remind: '',
				...
				template: function(elId, rowObject){
					return '<span class="plugin-action edit-action" onclick="showMsgbox(this)"  learnLink-id="'+rowObject.elId+'">编辑</span>'
							+'<span class="plugin-action del-action" onclick="delectRowData('+rowObject.elId+')" learnLink-id="'+rowObject.elId+'">删除</span>';
				}
			}
		]
		// 分页前事件
...
/* 		var tr = node.parentNode.parentNode;
		...
		//if(window.confirm('确认要删除该条数据吗?')){
			...
			var url="${pageContext.request.contextPath }/configuration/deleteData";
			 $.ajax({ 
					  type:'post', 
					  url:url,
					  data:{id:node},
					...
					  success:function(data,textStatus){
						...
						if(data.sucees=="success"){
							//用现有参数刷新，false 默认不回第一页
							table.GM('refreshGrid', false, function () {
							});
						}
					  }, 
...
function readConfig(node){
	...
	var tr = node.parentNode.parentNode;
	var rowData = document.getElementById("showTable").GM('getRowData', tr);
	 $(".pb_body").find("#elId").val(rowData.elId);
	 $(".pb_body").find("#elBadtype").val(rowData.elBadtype);
	 $(".pb_body").find("#elBadnum").val(rowData.elBadnum);
	 $(".pb_body").find("#elBadtypetime").val(new Date(rowData.elBadtypetime).format('YYYY-MM-DD HH:mm:ss'));
}	
...
<!-- 隐藏 域 弹层   -->
 <div id="msgBox" style="display: none;">
      		<form id="fm"  method="post" action="" enctype="multipart/form-data">
	  		  <table id="tableShow">
	  		  <tr> 
	  		  	<td class="label"><label class="Jleft" >ID:</label></td>
	  		  	<td class="field"><input id="elId" name="elId" type="text" value="" size="30" maxlength="200" /></td>
	  		  	<td class="label"><label class="Jleft" >不良类型:</label></td>
	  		  	<td class="field"><input id="elBadtype" name="elBadtype" type="text" size="30" value="" maxlength="200" /></td>
	  		  </tr>
	  		  <tr>
	  		  	 <td class="label"><label class="Jleft">阀值:</label></td>
	  		  	<td class="field"><input id="elBadnum" name="elBadnum" size="30" type="text" value="" maxlength="200" /></td>
	  		  	<td class="label"><label class="Jleft">数据时间:</label></td>
	  		  	<td class="field"><input id="elBadtypetime" name="elBadtypetime" size="30" type="text" value="" maxlength="200" /></td>
	  		  </tr>
	  		  </table>
